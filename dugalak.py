...
def cobalt_needed(gel_now, gel_need, react_volume):
    gel_diff = gel_now / gel_need
    return react_volume / 1200 * (gel_diff - 1)
...
def Choise_gel():
    ...
    if int(gel_now.get()) < int(gel_need.get()):
        ...
        result = cobalt_needed(int(gel_now.get()), int(gel_need.get()), int(react_volume.get()))
        answer.grid(row=6, column=1)
...
btn_lami = tk.Button(win, text = 'Вязкость ламинарная:', command = choise_viscozity_method)
...
btn_tixo.grid(row = 0, column = 1)
btn_gel.grid(row = 0, column = 2)
